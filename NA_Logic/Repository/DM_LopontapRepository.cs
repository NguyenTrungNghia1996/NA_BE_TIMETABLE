...
namespace NA_Logic.Repository
{
    public class DM_LopontapRepository : IDM_LopontapRepository
    {
        ...
        public List<DM_Lopontap_List> GetList_Paging(int PageIndex, int PageSize, string search, int idDonvi, int idKhoi, int idMon)
        {
            try
            {
                ...
                var result = _dbContext.Set<DM_Lopontap_List>().FromSqlRaw("EXEC DM_Lopontap_GetList_Paging @pageIndex, @pageSize, @search,@idDonvi, @idKhoi, @idMon",
                    paramPageIndex, paramPageSize, paramSearch, paramIdDonvi, paramIdKhoi, paramIdMon).ToList();
        ...
        public bool CheckContraint(int id, int idDonvi)
        {
            try
            {
                bool check = (from kq in _dbContext.KetQua_Baikiemtra.AsNoTracking()
                              ...
                              where lop.Id_don_vi == idDonvi && lop.Id == id
                              select 1).Any();
                return check;
            }
            catch
