...
namespace NA_Logic.Repository.LichOnTap
{
    public class Danhsach_LichontapRepository : IDanhsach_LichontapRepository
    {
        ...
        public bool CheckId_ChiTiet(int Id, int idDonvi)
        {
            ...
            try
            {
                bool ct = _context.Chitiet_Lichontap.Join(_context.Danhsach_Lichontap, c => c.Id_lich, d => d.Id, (c, d) => new { c.Id, Id_donvi = d.Id_don_vi }).Any(x => x.Id == Id && x.Id_donvi == idDonvi);
                return ct;
            }
            catch
            {
