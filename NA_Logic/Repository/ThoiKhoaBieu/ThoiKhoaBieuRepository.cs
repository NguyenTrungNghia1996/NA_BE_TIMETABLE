...
namespace NA_Logic.Repository.ThoiKhoaBieu
{
    public class ThoiKhoaBieuRepository : IDanhsach_ThoikhoabieuRepository
    {
        ...
        public bool CheckId_ChiTiet(int Id, int idDonvi)
        {
            ...
            try
            {
                bool ct =  _context.Chitiet_Thoikhoabieu.Join(_context.Danhsach_Thoikhoabieu, c=>c.Id_tkb, d=>d.Id, (c,d) => new {c.Id, Id_donvi = d.Id_don_vi}).Any(x=>x.Id==Id && x.Id_donvi==idDonvi);
                return ct;
            }
            catch
            {
