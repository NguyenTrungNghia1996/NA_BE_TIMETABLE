...
namespace NA_Xepthoikhoabieu.Controllers.ThoiKhoaBieu
{
    ...
    public class DM_PhonghocController : ControllerBase
    {
        ...
        public DM_PhonghocController(IMapper mapper, IDM_PhonghocRepository phonghoc, IClaimHelperRepository claimHelperRepository, IAuthRepository auth,
                                     IDM_LoaiphonghocRepository loaiphonghoc, IDM_DiemtruongRepository diemtruong, IDM_CahocRepository cahoc)
        {
            _mapper = mapper;
            _phonghoc = phonghoc;
        ...
        [HttpGet]
        [RequireToken]
        public IActionResult GetList_Paging([FromQuery] int PageIndex, [FromQuery] int PageSize,[FromQuery] int idLoaiPhonghoc, [FromQuery] string search = "", [FromQuery] int idDiemTruong=0)
        {
        ...
        public IActionResult Update([FromBody] Phong_banDto phongban)
        {
            ...
            //add
            bool result = _phonghoc.AddTietBan(danhSachTietBan, phongban.Id, phongban.apply_all, idDonvi);
            if (!result)
