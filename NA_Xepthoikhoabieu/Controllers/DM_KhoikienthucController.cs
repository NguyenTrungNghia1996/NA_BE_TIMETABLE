...
namespace NA_Xepthoikhoabieu.Controllers
{
    ...
    public class DM_KhoikienthucController : ControllerBase
    {
        ...
        public IActionResult GetList_Paging([FromQuery] int PageIndex, [FromQuery] int PageSize, [FromQuery] string search = "")
        {
            ...
            int totalrecord = 0;
            search = search.Trim();
            var list = _khoikienthuc.GetList_Paging(PageIndex, PageSize, search,idDonvi, ref totalrecord);
        ...
        public IActionResult GetDetailByID([FromQuery] int Id)
        {
            ...
            if (idDonvi == 0) return ApiResult.Unauthorized("Thông tin đơn vị không hợp lệ, vui lòng kiểm tra lại hoặc liên hệ admin để biết thêm chi tiết");
            // Lấy bản ghi từ db
            var detail = _khoikienthuc.GetDetailById(Id,idDonvi);
            if (detail == null)
        ...
        public IActionResult Update([FromBody] DM_KhoikienthucDto khoikienthuc)
        {
            ...
            var db = _khoikienthuc.GetDetailById(khoikienthuc.Id, idDonvi);
            if (!ModelState.IsValid)
        ...
        public IActionResult Delete([FromQuery] int id)
        {
            ...
            int idDonvi = _claimHelperRepository.GetIdDonvi(User);
            if (idDonvi == 0) return ApiResult.Unauthorized("Thông tin đơn vị không hợp lệ, vui lòng kiểm tra lại hoặc liên hệ admin để biết thêm chi tiết");
            var item = _khoikienthuc.GetDetailById(id, idDonvi);
            if (item == null)
                return ApiResult.NotFound($"Bản ghi có Id= {id} không tồn tại, vui lòng kiểm tra lại");
            var (success, message) = _khoikienthuc.Delete(id);
            if (!success)
